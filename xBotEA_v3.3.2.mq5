//+------------------------------------------------------------------+
//|                                                xBotEA_v3.2.3.mq5 |
//|  3.2.2 + hàng đợi xác minh đóng bất đồng bộ (OnTimer) khi         |
//|  ClosePosition cạn retry — tránh chặn luồng tín hiệu chính.    |
//+------------------------------------------------------------------+
//|                                                xBotEA_v3.3.2.mq5 |
//|  Bản 3.3.2 — cùng logic 3.3.1 (3.2.3)                          |
//+------------------------------------------------------------------+
...
input int       CloseVerifyIntervalMs = 300;  // 3.2.3: khoảng cách thử lại lệnh đóng sót (ms)
input int       CloseVerifyMaxAttempts = 25;  // 3.2.3: tối đa số lần thêm sau khi ClosePosition thất bại
...
datetime g_lastStatusFileWrite = 0; // Throttle ghi status.txt (Client đọc balance đích + số lệnh mở)
...
// 3.2.3: tác vụ kiểm tra/đóng bù (chạy từ OnTimer, không chặn ProcessSignal)
struct CloseVerifyTask
{
   ulong  masterTicket;
   string symbol;
   int    attempts;
...
CloseVerifyTask g_closeVerifyQ[];
const int g_closeVerifyPerTimer = 4;

bool CloseVerifyIsDuplicate(ulong masterTicket, string sym)
{
   for (int i = 0; i < (int)ArraySize(g_closeVerifyQ); i++)
   {
      if (g_closeVerifyQ[i].masterTicket == masterTicket && g_closeVerifyQ[i].symbol == sym)
         return true;
   }
   return false;
}

void CloseVerifyEnqueue(ulong masterTicket, string sym)
{
   if (CloseVerifyIsDuplicate(masterTicket, sym)) return;
   if ((int)ArraySize(g_closeVerifyQ) >= 96)
   {
      Print("CloseVerify: queue full, bỏ qua thêm lệnh master ", masterTicket);
      return;
   }
   int n = (int)ArraySize(g_closeVerifyQ);
   ArrayResize(g_closeVerifyQ, n + 1);
   g_closeVerifyQ[n].masterTicket = masterTicket;
   g_closeVerifyQ[n].symbol = sym;
   g_closeVerifyQ[n].attempts = 0;
   g_closeVerifyQ[n].nextMs = (uint)(GetTickCount() + 200);
   Print("CloseVerify: enqueued master ", masterTicket, " ", sym);
}

void ProcessCloseVerifyQueue()
{
   uint now = GetTickCount();
   int processed = 0;
   int k = 0;
   while (k < (int)ArraySize(g_closeVerifyQ) && processed < g_closeVerifyPerTimer)
   {
      if (g_closeVerifyQ[k].nextMs > now) { k++; continue; }
      
      ulong mt = g_closeVerifyQ[k].masterTicket;
      string sym = g_closeVerifyQ[k].symbol;
      bool hasMatch = false;
      for (int j = PositionsTotal() - 1; j >= 0; j--)
      {
         ...
         if (!IsMatchMasterTicketComment(c, mt)) continue;
         hasMatch = true;
         if (trade.PositionClose(t))
         {
            int sz = ArraySize(g_reports);
            ArrayResize(g_reports, sz + 1);
            g_reports[sz].masterTicket = mt;
            g_reports[sz].slaveTicket = t;
            g_reports[sz].closeTime = TimeCurrent();
            g_reports[sz].symbol = sym;
            g_reports[sz].comment = c;
            Print("CloseVerify: closed slave ", t, " master ", mt);
            for (int r2 = k; r2 < (int)ArraySize(g_closeVerifyQ) - 1; r2++)
               g_closeVerifyQ[r2] = g_closeVerifyQ[r2 + 1];
            ArrayResize(g_closeVerifyQ, ArraySize(g_closeVerifyQ) - 1);
            processed++;
            continue;
         }
         Print("CloseVerify: close fail ", t, " ", trade.ResultRetcodeDescription());
         break;
      }
      
      if (!hasMatch)
      {
         for (int r = k; r < (int)ArraySize(g_closeVerifyQ) - 1; r++)
            g_closeVerifyQ[r] = g_closeVerifyQ[r + 1];
         ArrayResize(g_closeVerifyQ, ArraySize(g_closeVerifyQ) - 1);
         processed++;
         continue;
      }
      g_closeVerifyQ[k].attempts++;
      if (g_closeVerifyQ[k].attempts >= CloseVerifyMaxAttempts)
      {
         Print("CloseVerify: bỏ cuộc master ", mt, " ", sym, " (max attempts)");
         for (int r3 = k; r3 < (int)ArraySize(g_closeVerifyQ) - 1; r3++)
            g_closeVerifyQ[r3] = g_closeVerifyQ[r3 + 1];
         ArrayResize(g_closeVerifyQ, ArraySize(g_closeVerifyQ) - 1);
      }
      else
         g_closeVerifyQ[k].nextMs = (uint)(now + (uint)CloseVerifyIntervalMs);
      k++;
      processed++;
   }
}
...
int OnInit()
{
   ...
   Print("========================================");
   Print("xBotEA v3.3.2 initialized (CloseVerify background queue)");
   Print("Signal file: ", SignalFile);
   Print("Poll interval: ", PollInterval, "ms");
   Print("========================================");
   
   // Một lần khi khởi động — Client cần status.txt ngay
   UpdateStatusFile();
...
void OnTimer()
{
   // 3.2.3: xử lý tác vụ đóng bù (không chặn khi cần tốc độ — giới hạn số tác mỗi lần)
   ProcessCloseVerifyQueue();
   ...
   // Có tín hiệu: ghi status ngay để Client khóa turnTargetBalance đúng balance đích; không tín hiệu: tối đa ~1 lần/giây
   datetime nowT = TimeCurrent();
   if (content != "") {
      UpdateStatusFile();
      g_lastStatusFileWrite = nowT;
   } else if (nowT - g_lastStatusFileWrite >= 1) {
      UpdateStatusFile();
      g_lastStatusFileWrite = nowT;
   }
...
void UpdateStatusFile()
{
   ...
   if(h != INVALID_HANDLE) {
      // id,balance,equity,positionsTotal — Client dùng cột 4 để biết hết lệnh (reset turn)
      FileWriteString(h, (string)g_accountID + "," + DoubleToString(balance, 2) + "," + DoubleToString(equity, 2) + "," + IntegerToString(openPos));
      FileClose(h);
   }
}
...
bool ClosePosition(string symbol, ulong masterTicket)
{
   ...
   for (int pass = 0; pass < MAX_PASS; pass++)
   {
      ...
      if (stagnantPass >= 20)
      {
         ...
         CloseVerifyEnqueue(masterTicket, symbol);
         return false;
      }
      Sleep(100);
   }
