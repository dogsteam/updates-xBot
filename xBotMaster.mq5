...
input bool     InpLegOnlyChartSymbol = true; // Khôi phục chuỗi leg chỉ lệnh cùng symbol chart (_Symbol)
...
// Chuỗi turn DCA: nối B (BUY) / S (SELL) theo thứ tự mỗi lệnh mở; đảo chiều vẫn nối (vd BBS). Hết lệnh (flat) → reset.
string g_legSB = "";
ulong  g_legPositionIds[]; // POSITION_IDENTIFIER đã đếm 1 ký tự leg (tránh khớp từng phần = nhiều DEAL_IN cùng position)

void LegPositionIdsClear()
{
   ArrayResize(g_legPositionIds, 0);
}

bool LegPositionIdsContains(ulong pid)
{
   if (pid == 0) return false;
   for (int i = 0; i < ArraySize(g_legPositionIds); i++) {
      if (g_legPositionIds[i] == pid) return true;
   }
   return false;
}

void LegPositionIdsAdd(ulong pid)
{
   if (pid == 0) return;
   if (LegPositionIdsContains(pid)) return;
   int n = ArraySize(g_legPositionIds);
   ArrayResize(g_legPositionIds, n + 1);
   g_legPositionIds[n] = pid;
}

bool LegPositionIdsRemove(ulong pid)
{
   if (pid == 0) return false;
   for (int i = 0; i < ArraySize(g_legPositionIds); i++) {
      if (g_legPositionIds[i] == pid) {
         int last = ArraySize(g_legPositionIds) - 1;
         g_legPositionIds[i] = g_legPositionIds[last];
         ArrayResize(g_legPositionIds, last);
         return true;
      }
   }
   return false;
...
//+------------------------------------------------------------------+
//| Khôi phục g_legSB từ lệnh đang mở (EA restart giữa turn)          |
//| Sắp xếp theo thời gian mở tăng dần; cùng giây → ticket tăng dần.   |
//+------------------------------------------------------------------+
void RebuildLegSBFromOpenPositions()
{
   g_legSB = "";
   LegPositionIdsClear();
   int n = PositionsTotal();
   if (n <= 0) {
      Print("RebuildLegSB: không có lệnh mở — leg rỗng.");
      return;
   }
   ...
   long types[];
   int cap = n + 8;
   ArrayResize(tickets, cap);
   ArrayResize(times, cap);
   ArrayResize(types, cap);
   int count = 0;
   ...
   for (int i = 0; i < n; i++) {
      ...
      if (InpLegOnlyChartSymbol && sym != _Symbol) continue;

      string comm = PositionGetString(POSITION_COMMENT);
      if (StringFind(comm, "CopyTrade") >= 0) continue;
   ...
   if (count <= 0) {
      Print("RebuildLegSB: không có lệnh khớp (symbol/CopyTrade) — leg rỗng.");
      return;
   }

   // Sắp xếp: time tăng, trùng time → ticket tăng
   for (int a = 0; a < count - 1; a++) {
      for (int b = a + 1; b < count; b++) {
         bool swap = false;
         if (times[b] < times[a]) swap = true;
         else if (times[b] == times[a] && tickets[b] < tickets[a]) swap = true;
         if (swap) {
            datetime t0 = times[a]; times[a] = times[b]; times[b] = t0;
            ulong k0 = tickets[a]; tickets[a] = tickets[b]; tickets[b] = k0;
            long y0 = types[a]; types[a] = types[b]; types[b] = y0;
         }
      }
   }
   ...
   for (int j = 0; j < count; j++) {
      ...
      LegPositionIdsAdd(pid);
      if (types[j] == POSITION_TYPE_BUY) g_legSB += "B";
      else if (types[j] == POSITION_TYPE_SELL) g_legSB += "S";
   }

   Print("RebuildLegSB: ", count, " lệnh mở → leg=\"", g_legSB, "\" (chart=", _Symbol, ")");
}
                        ...
                        const MqlTradeResult& result)
{
   ...
   // Handle OPEN signal (DEAL_ENTRY_IN)
      ...
      // Khớp từng phần: nhiều DEAL_IN cùng position_id → chỉ deal đầu nối leg + 1 signal (tránh leg BBB + signal thừa)
      if (positionId > 0 && LegPositionIdsContains(positionId)) {
         Print("Skip IN (partial fill duplicate): positionId=", positionId, " deal=", dealTicket);
         return;
      }
      ...
      // Mỗi lệnh (position) mở: 1 ký tự — sau khi append ghi nhận positionId để lần IN tiếp theo cùng position bị bỏ qua
      string ch = (dealType == DEAL_TYPE_BUY) ? "B" : "S";
      g_legSB += ch;
      if (positionId > 0) LegPositionIdsAdd(positionId);
   } else {
      // Unknown entry type, skip
      return;
   }
   
   // Build signal: Ticket,Symbol,Type,Volume,SL,TP,LegSB,BalanceNguon — Client khóa B_turn từ lệnh đầu; mọi lệnh mở đều gửi balance (Client chỉ dùng lần đầu khi chưa khóa).
   string legField = "";
   string balField = "";
   if (dealEntry == DEAL_ENTRY_IN && (orderType == "BUY" || orderType == "SELL")) {
      legField = g_legSB;
      balField = DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2);
   }
   ...
   if (StringLen(signal) > 0) {
      ...
      if (SendHttpPost("/update", signal)) {
         ...
         if (dealEntry == DEAL_ENTRY_OUT && PositionsTotal() == 0) {
            g_legSB = "";
            LegPositionIdsClear();
            Print("Leg SB reset (no open positions).");
         } else if (dealEntry == DEAL_ENTRY_OUT && positionId > 0) {
            if (!PositionSelectByTicket(positionId))
               LegPositionIdsRemove(positionId);
         }
      } else {
         Print("Failed to send signal: ", signal, " - Check server is running on ", TargetIP, ":", TargetPort);
         if (dealEntry == DEAL_ENTRY_IN && positionId > 0) {
            LegPositionIdsRemove(positionId);
            int slen = StringLen(g_legSB);
            if (slen > 0) g_legSB = StringSubstr(g_legSB, 0, slen - 1);
            Print("Rolled back leg append for failed HTTP, positionId=", positionId);
         }
      }
   }
}
